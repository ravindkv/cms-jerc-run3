...
int HistMCTruth::Run(SkimTree *tree, EventPick *eventP, ObjectPick *objP, ObjectScale *objS, TFile *fout){
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for (int i = 0; i != tree->nJet; ++i) {
      ...
      //compute the L1RC correction and apply to the uncorrected jet;
      ...
      p4Jet.SetPtEtaPhiM(tree->Jet_pt[i], tree->Jet_eta[i], tree->Jet_phi[i], tree->Jet_mass[i]);
		...
		for (UInt_t genJetIndex = 0; genJetIndex != tree->nGenJet; ++genJetIndex) {
		  ...
		  if (hasMatchVtx && hasMatchJet) {
		    ...
		    p2ResponseInJetEtaGenJetPt->Fill(
		    		std::fabs(p4Jet.Eta()), p4GenJet.Pt(), 
		    		p4Jet.Pt() / p4GenJet.Pt(), weight
		    );
		    ...
		    for (size_t etaBinIdx = 0; etaBinIdx < eta_bins.size(); ++etaBinIdx) {
		      if (std::fabs(p4Jet.Eta()) >= eta_bins[etaBinIdx].first && 
		        ...
		        pResponseInGenJetPt[etaBinIdx]->Fill(p4GenJet.Pt(), p4Jet.Pt() / p4GenJet.Pt());
		        hResponse[etaBinIdx]->Fill(p4Jet.Pt() / p4GenJet.Pt());
		        break;
		      }
		    }
		  }//hasMatch
